...
class MainTitleController extends Controller
{
   ...
   use HasResourceActions;
    ...
    {
        $show = new Show(MainTitle::findOrFail($id));
    ...
    protected function form()
    {
		...
		$form->number('place', '排序')->required()->min(1)->default(1)->rules(function ($form) {
			...
			if ($id = $form->model()->id) {
				return "unique:main_title,place,$id,id";
			}
			if (!$id = $form->model()->id) {
				return 'unique:main_title,place';
			}
		},['警告:已有相同排序，请重新设定']);
        return $form;
    }
